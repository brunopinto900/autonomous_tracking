    ...
    ######### PLOTTING #########
        ...
        if not pause:
            ...
            if(len(target_x) <= 1): # first time, initialize with drone's initial state
                posfX, posfY, tangent = getGoalPosition(x[0], x[1],target_xGoal,target_yGoal, DIST_2_TARGET, x[0], x[1], 1.42)
            else:
                posfX, posfY, tangent = getGoalPosition(x[0], x[1],target_xGoal,target_yGoal, DIST_2_TARGET,
                                               target_x[-2], target_y[-2], tangent)
